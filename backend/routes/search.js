...
router.get("/:symbols", async (req, res, next) => {
  ...
  for (let i = 0; i < symbolsString.length; i++) {
    if (symbols.includes(symbolsString.charAt(i))) {
      res.status(400).json("Bad Request");
      return;
    }
  }
...
router.get("/string/:symbols", async (req, res, next) => {
  const symbols = req.params.symbols;
  let symbolsString = "!@#$%''^&*()_+=-\\|][}{';:/?><.,`~";
  for (let i = 0; i < symbolsString.length; i++) {
    if (symbols.includes(symbolsString.charAt(i))) {
...
router.get("/MainDisplay/:symbols", async (req, res, next) => {
  const symbols = req.params.symbols;
  let symbolsString = "!@#$%''^&*()_+=-\\|][}{';:/?><.,`~";
  for (let i = 0; i < symbolsString.length; i++) {
    if (symbols.includes(symbolsString.charAt(i))) {
...
router.get("/AdvancedDisplay/:symbols", async (req, res, next) => {
  const symbols = req.params.symbols;
  let symbolsString = "!@#$%''^&*()_+=-\\|][}{';:/?><.,`~";
  for (let i = 0; i < symbolsString.length; i++) {
    if (symbols.includes(symbolsString.charAt(i))) {
...
router.get("/hotel/prices/:symbols", async (req, res, next) => {
  const symbols = req.params.symbols;
  let symbolsString = "!@#$%''^&*()_+=-\\|][}{';:/?><.,`~";
  for (let i = 0; i < symbolsString.length; i++) {
    if (symbols.includes(symbolsString.charAt(i))) {
