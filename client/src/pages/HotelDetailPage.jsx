...
export default function HotelDetailPage() {
  ...
  // 🔁 NEW: trigger a re-render when auth changes (same-tab & cross-tab)
  ...
  // 🔁 NEW: re-render automatically when login/logout happens
  useEffect(() => {
    // cross-tab (fires in other tabs/windows)
    const onStorage = (e) => {
    ...
    // same-tab polling (storage doesn't fire in same tab)
    let last = `${localStorage.getItem("token") || ""}|${
  ...
  function initPage() {
    ...
    const defCheckIn = new Date();
    const defCheckOut = new Date(); // ✅ fixed typo
