...
// // Mock react-router-dom's useNavigate at top level to avoid redefine error
...
//     // Flexible apostrophe matcher (’ or ')
//     await waitFor(() => {
//       expect(
...
//   // --- New test for loading state on remove ---
//   test("shows loading state during bookmark removal", async () => {
//     render(
//       <MemoryRouter>
//         <Bookmark />
//       </MemoryRouter>
...
//     // Wait for initial bookmarks to render
//     await waitFor(() => {
//       expect(screen.getByText("Hotel One")).toBeInTheDocument();
//     });

//     // Mock getBookmarkedHotels to delay during removal fetch
//     let resolveGet;
//     const delayedPromise = new Promise((res) => (resolveGet = res));
//     bookmarkApi.getBookmarkedHotels.mockReturnValueOnce(delayedPromise);

//     const removeButtons = screen.getAllByText("❌ Remove");

//     // Click remove triggers loading state (since bookmarks empty before resolve)
//     await act(async () => {
//       fireEvent.click(removeButtons[0]);
//     });

//     // Loading should appear because bookmarks are empty before fetch resolves
//     expect(screen.getByText(/loading/i)).toBeInTheDocument();

//     // Now resolve the delayed getBookmarkedHotels call with updated data
//     resolveGet({ data: [bookmarkApi.getBookmarkedHotels.mock.calls[0][0]] });

//     // Wait for loading to disappear and remaining bookmarks to appear
//     await waitFor(() => {
...
//   // --- New test for edge case: bookmark with missing fields ---
//   test("handles bookmarks with missing images or ratings gracefully", async () => {
//     // Setup bookmarks with missing fields
...
//           hotel_ratings: 0, // No ratings
//           stitchedImageUrls: [], // Empty images
...
//     // Check the fallback image for missing images
//     const images = screen.getAllByRole("img");
//     // Filter out the logo image that shows as "mock-logo.png"
...
//     // Check the image url for the hotel with image present
//     expect(hotelImages[1]).toHaveAttribute(
//       "src",
//       "https://images.test/hotel4.jpg"
//     );

//     // Check the stars rendering for missing or zero ratings
//     // For "Hotel Missing Image" rating 0, should render empty string
//     expect(
//       screen.getByText("Hotel Missing Image").nextSibling.nextSibling
//         .textContent
//     ).toBe("");

//     // For "Hotel Missing Ratings" (field missing), repeat() defaults to 0 stars (empty)
//     expect(
